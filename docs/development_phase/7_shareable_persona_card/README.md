...
| Asset | Status | Notes |
|---|---|---|
| `lib/finance/persona.ts` | ✅ updated | `PERSONA_VISUALS` added as source-of-truth for gradient + emoji |
| `lib/copy/strings.ts` | ✅ updated | "Sobat Indomie" → "Sobat Mie" (display-only). 13 new share + landing copy strings |
| `composables/useShare.ts` | ✅ refactored | Generic Layer 1 — no `PersonaKey` dependency. Exports: `captureAsPng`, `copyText`, `shareToWa`, `shareToTwitter`, `shareNative`, `isMobileShareCapable` |
| `components/common/ShareDialog.vue` | ✅ new (Layer 2) | Generic share modal — inline overlay (no Teleport), slot-based, native share on mobile / grid on desktop |
| `components/share/PersonaShareCard.vue` | ✅ new (Layer 3) | Persona card visual — props-only (no store access), vertical-friendly layout, stats default OFF |
...
| `tests/components/PersonaShareCard.test.ts` | ✅ new | 8 privacy tests — no digits in labels/taglines/CTA, greylist boundary, share text audit |
| `tests/finance/persona.test.ts` | ✅ updated | Updated for "Sobat Mie" + PERSONA_VISUALS test suite |
