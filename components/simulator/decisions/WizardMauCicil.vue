...
// "Mau Cicil?" wizard UI. Mirror WizardMauKpr but with tipe dropdown (6 non-KPR options)
  ...
  <div class="space-y-6">
    <header>
      <h2 class="text-lg font-semibold text-[var(--color-text-primary)]">
        {{ t('wizard.cicil.title') }}
      </h2>
      <p class="mt-1 text-sm text-[var(--color-text-secondary)]">
        {{ t('wizard.cicil.subtitle') }}
      </p>
    </header>

    <section class="grid gap-3 sm:grid-cols-2">
      <label class="block text-xs sm:col-span-2">
        <span class="font-medium text-[var(--color-text-secondary)]">{{ t('wizard.cicil.form.label') }}</span>
        <input
          v-model="label"
          type="text"
          :placeholder="t('wizard.cicil.form.labelPlaceholder')"
      ...
      <label class="block text-xs">
        <span class="font-medium text-[var(--color-text-secondary)]">{{ t('wizard.cicil.form.tipe') }}</span>
        <select
          v-model="tipe"
          class="mt-1 h-10 w-full rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-low)] px-3 text-sm text-[var(--color-text-primary)] outline-none focus:border-[var(--color-primary)]"
        >
      ...
      <label class="block text-xs">
        <span class="font-medium text-[var(--color-text-secondary)]">{{ t('wizard.cicil.form.harga') }}</span>
        <div class="mt-1">
      ...
      <label class="block text-xs">
        <span class="font-medium text-[var(--color-text-secondary)]">{{ t('wizard.cicil.form.dp') }}</span>
        <input
      ...
      <label class="block text-xs">
        <span class="font-medium text-[var(--color-text-secondary)]">{{ t('wizard.cicil.form.tenor') }}</span>
        <input
      ...
      <label class="block text-xs">
        <span class="font-medium text-[var(--color-text-secondary)]">{{ t('wizard.cicil.form.bunga') }}</span>
        <input
      ...
      <label class="block text-xs">
        <span class="font-medium text-[var(--color-text-secondary)]">{{ t('wizard.cicil.form.jenisBunga') }}</span>
        <select
          v-model="jenisBunga"
          class="mt-1 h-10 w-full rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-low)] px-3 text-sm text-[var(--color-text-primary)] outline-none focus:border-[var(--color-primary)]"
        >
      ...
      <label class="block text-xs sm:col-span-2">
        <span class="font-medium text-[var(--color-text-secondary)]">{{ t('wizard.cicil.form.asetKategori') }}</span>
        <select
          v-model="asetKategori"
          class="mt-1 h-10 w-full rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var(--color-surface-low)] px-3 text-sm text-[var(--color-text-primary)] outline-none focus:border-[var(--color-primary)]"
        >
          <option value="">{{ t('wizard.cicil.form.asetSkip') }}</option>
          <option v-for="k in ASET_KATEGORI_OPTIONS" :key="k" :value="k">{{ k }}</option>
        </select>
        <p class="mt-1 text-[11px] text-[var(--color-text-muted)]">{{ t('wizard.cicil.form.asetHint') }}</p>
      </label>

      <label v-if="asetKategori !== ''" class="block text-xs sm:col-span-2">
        <span class="font-medium text-[var(--color-text-secondary)]">{{ t('wizard.cicil.form.asetValue') }}</span>
        <div class="mt-1">
    ...
    <template v-if="result && summary">
      <section class="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface-low)] p-4">
        <h3 class="mb-2 text-sm font-semibold text-[var(--color-text-primary)]">
          {{ t('wizard.cicil.summary.title') }}
        </h3>
        <ul class="grid gap-1 text-sm text-[var(--color-text-secondary)] sm:grid-cols-2">
          <li>{{ t('wizard.cicil.summary.dp', { amount: idr(summary.dp) }) }}</li>
          <li>{{ t('wizard.cicil.summary.pokok', { amount: idr(summary.pokok) }) }}</li>
          <li>{{ t('wizard.cicil.summary.cicilan', { amount: idr(summary.cicilan) }) }}</li>
          <li>{{ t('wizard.cicil.summary.totalBunga', { amount: idr(summary.totalBunga) }) }}</li>
        </ul>
      </section>
