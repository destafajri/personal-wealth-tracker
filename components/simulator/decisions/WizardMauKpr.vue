...
// "Mau KPR?" wizard UI. Three states:
...
//
// Wizard purity: this component never mutates the snapshot store. It pulls a frozen
...
import { runMauKpr, type KprInput } from '~/lib/finance/wizards/mau-kpr'
...
function goalImpactMessage(g: GoalDelta): string {
  ...
  if (g.monthsShift === 0) return t('wizard.goalImpact.shift.none', { label: g.goalLabel })
  ...
  <div class="space-y-6">
    ...
    <section class="grid gap-3 sm:grid-cols-2">
      <label class="block text-xs sm:col-span-2">
        <span class="font-medium text-[var(--color-text-secondary)]">
          {{ t('wizard.kpr.form.label') }}
        </span>
        <input
          v-model="label"
          type="text"
          :placeholder="t('wizard.kpr.form.labelPlaceholder')"
    ...
    <div class="flex flex-wrap items-center gap-3">
      <ButtonPrimary :disabled="!canSubmit" @click="submit">
        {{ t('wizard.kpr.form.submit') }}
      </ButtonPrimary>
      <ButtonGhost v-if="result" @click="reset">
        {{ t('wizard.kpr.form.reset') }}
      </ButtonGhost>
    </div>
    ...
    <template v-if="result && summary">
      ...
      >
        <h3 class="mb-2 text-sm font-semibold text-[var(--color-text-primary)]">
          {{ t('wizard.kpr.summary.title') }}
        </h3>
        <ul class="grid gap-1 text-sm text-[var(--color-text-secondary)] sm:grid-cols-2">
          <li>{{ t('wizard.kpr.summary.dp', { amount: idr(summary.dp) }) }}</li>
          <li>{{ t('wizard.kpr.summary.pokok', { amount: idr(summary.pokok) }) }}</li>
          <li>{{ t('wizard.kpr.summary.cicilan', { amount: idr(summary.cicilan) }) }}</li>
          <li>{{ t('wizard.kpr.summary.totalBunga', { amount: idr(summary.totalBunga) }) }}</li>
        </ul>
      </section>
